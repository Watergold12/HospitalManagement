...
export default function Doctors() {
  ...
  const handleEdit = (d) => {
    ...
    setFormData({
      ...
      license_no: d.licenseno || '',   // ✅ fixed mapping
      dept_id: d.dept_id || ''
    });
  };
      ...
      <form onSubmit={handleSubmit} className="modern-form">
        ...
        <input
          ...
          onChange={e =>
            setFormData({
              ...formData,
              dept_id: e.target.value ? parseInt(e.target.value) : ''
            })
          }
        />
      ...
      <div className="table-container" style={{ marginTop: '2rem' }}>
        <table className="modern-table">
          ...
          <tbody>
            {data.map(item => (
              <tr key={item.doctor_id}>
                ...
                <td style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => handleEdit(item)} className="btn" style={{ background: 'var(--accent)', color: 'white' }}>
                    <Edit2 size={16} /> Edit
                  </button>
            ...
            {data.length === 0 && (
              <tr>
                <td colSpan="7" className="empty-state">No doctors found.</td>
              </tr>
            )}
          </tbody>
        </table>
